...
// Removed HeaderProps interface as toggleSidebar is no longer passed
export default function Header() { // Removed toggleSidebar prop
    ...
    <header className="fixed top-0 left-0 right-0 z-10 bg-gray-900 bg-opacity-75 shadow-lg">
      <div className="container mx-auto px-6 py-4 flex items-center justify-between">
        <div className="flex-shrink-0">
          <div className="text-2xl font-press-start text-yellow-400">
            <Link href="/">Document Management</Link>
          </div>
        </div>

        {/* Left: Menu Bar */}
        <nav className="flex items-center space-x-8"> {/* Always visible desktop nav */}
          {isLoggedIn && (
            <>
              <Link
                href="/documents"
        ...
        <div className="flex items-center space-x-4"> {/* Combined Logo and User Profile */}
          <div className="flex-shrink-0">
            {isLoggedIn && user ? (
